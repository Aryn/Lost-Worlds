...
/character
	...
	var/char_form/form //Allows clothes to fit differently on different characters.
...
/character/proc/TryOperate(atom/movable/target)
	...
	if(!InRangeOf(target))
		...
		else if(!target.AppliedBy(src,active_slot.item))
...
/character/proc/TryRangedOperate(atom/movable/target)
	if(active_slot.item)
		if(!target.AppliedLongRange(src,active_slot.item))
...
/character/proc/GetContext() //Returns command context.
	return ALL

/character/proc/ActiveItem()
	return active_slot ? active_slot.item : null