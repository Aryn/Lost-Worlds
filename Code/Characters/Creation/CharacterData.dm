...
/data/save
	var/name       //string
	var/character_form/form  //list-index
	var/age        //word
	...
	proc/Save(binfile/f)
		...
		f.putstring(name)
		f.putword(all_forms.Find(form.name))
		f.putword(age)
		CHECK_FILE_ERRORS

		f.putword(form.hair_images.Find(hair_style))
		f.putword(all_beards.Find(beard))
	...
	proc/WriteToPlayer(character/humanoid/human/P)
		...
		var/image/i_beard = all_beards[beard]
	...
	proc/Randomize()
		form = all_forms[pick(all_forms)]
		if(form.name == "female")
			name = "[pick(random_names_female)] [pick(random_names_last)]"
		else
			name = "[pick(random_names_male)] [pick(random_names_last)]"
		age = rand(25,75)
		hair_style = pick(form.hair_images)
		if(form.bearded && prob(50)) beard = pick(all_beards)
		else beard = "None"
