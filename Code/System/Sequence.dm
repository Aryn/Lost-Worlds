...
sequence/var/list/nodes
sequence/var/target

sequence/var/process/top
sequence/var/index = 1

sequence/New(target, list/nodes)
	src.target = target
	src.nodes = nodes
	top = nodes[1]

sequence/proc/Try(character/C, item/I)
	if(!istype(I,top.tool)) return 0

	view(C) << dd_replacetext(dd_replacetext(dd_replacetext(top.msg, "/user", C.name), "/tool", "[I]"), "/target", "[target]")
	spawn(top.usage_period)
		if(C.last_moved > world.time - top.usage_period) return 1
		call(target, top.cmd)(C,I)
		top = nodes[++index]
		return 1
